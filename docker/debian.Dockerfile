...
RUN set -xe && \
    ...
    apt-get install -y $(echo $(wget --no-check-certificate -qO- https://raw.githubusercontent.com/jerrita/nas-tools/alpha/package_list_debian.txt)) && \
    ...
    pip install cython && \
    pip install -r https://raw.githubusercontent.com/jerrita/nas-tools/alpha/requirements.txt && \
    # Clear
...
ENV S6_SERVICES_GRACETIME=30000 \
    ...
    NASTOOL_VERSION=alpha \
    REPO_URL="https://github.com/jerrita/nas-tools.git" \
    DISCARD_PYPI_MIRROR="https://pypi.tuna.tsinghua.edu.cn/simple" \
    PUID=0 \
    PGID=0 \
    UMASK=000 \
...
RUN set -xe \
    ...
    && git clone -b alpha ${REPO_URL} ${WORKDIR} --depth=1 --recurse-submodule \
